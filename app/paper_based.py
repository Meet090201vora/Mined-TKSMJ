...
# Abstract
    ...
    for i in l:
        ...
        with st.expander("Abstract"):
            ...
            ch=['0','1','2','3','4','5']
            rating=st.selectbox('How you like the suggestion '+str(i),ch)
            t=st.form_submit_button('submit '+str(i))
            print(t)
            print(rating)

            if(t):
                print(rating)
                df.loc[user,i+1]=int(rating)
                print(df.loc[user,i+1])
            else:
                # st.write('he')
                pass
...
def app():
    ...
    with st.form(key='searchform'):
        ...
        # st.write(id)
        if search:
            rate=output(paper_based_filtering.app(user,id),user)
