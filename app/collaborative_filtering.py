...
def app(user,id):
    ...
    arr = arr/len(x)
    # arr
    for i in range(len(df['user_'+str(id)])):
        if df['user_'+str(id)][i] >= 3:
            arr[0][i] = max(df['user_'+str(id)][i],arr[0][i])
        if df['user_'+str(id)][i] < 3 and df['user_'+str(id)][i] != 0:
            arr[0][i] = min(df['user_'+str(id)][i],arr[0][i])
